...
module.exports = function(argv){
	...
	// destination path
	const dir = Path.resolve(process.cwd(), argv.path);

	// create dir
	mkDir(dir)
		.catch(err=>{
			console.log('-'.repeat(60).yellow);
			console.log('The directory already exists, cannot create project in existing directory'.red);
			console.log('-'.repeat(60).yellow);
			setImmediate(()=>{
				process.exit(1);
			});
		...
		.then(()=> Promise.all([
				...
				FSE.copy(Path.resolve(__dirname, '../app/hgignore'), 		Path.resolve(dir, '.hgignore')),
				FSE.copy(Path.resolve(__dirname, '../app/gitignore'), 		Path.resolve(dir, '.gitignore')),
		...
		.catch(err=>{
			console.log('-'.repeat(60).yellow);
			console.log('An unknown error occurred'.red);
			console.log('-'.repeat(60).yellow);
			setImmediate(()=>{
				process.exit(1);
			});
